//Library from: https://github.com/pazi88/CANBus

#include "CANBus.h"

constexpr Baudrate_entry_t CANBus::BAUD_RATE_TABLE_48M[];
constexpr Baudrate_entry_t CANBus::BAUD_RATE_TABLE_45M[];

static CANBus* _CAN1 = nullptr;
static CAN_HandleTypeDef     hcan1;
uint32_t test = 0;
#ifdef CAN2
static CANBus* _CAN2 = nullptr;
static CAN_HandleTypeDef     hcan2;
#endif
#ifdef CAN3
static CANBus* _CAN3 = nullptr;
static CAN_HandleTypeDef     hcan3;
#endif

CANBus::CANBus( CAN_TypeDef* canPort, CAN_PINS pins, RXQUEUE_TABLE rxSize, TXQUEUE_TABLE txSize ) {
...
// Init and start CAN
void CANBus::begin( bool retransmission ) {
...
void CANBus::setBaudRate(uint32_t baud)
{
...
bool CANBus::write(CAN_message_t &CAN_tx_msg, bool sendMB)
{
  bool ret = true;
  uint32_t TxMailbox;
...
bool CANBus::read(CAN_message_t &CAN_rx_msg)
{
  bool ret;
...
bool CANBus::setFilter(uint8_t bank_num, uint32_t filter_id, uint32_t mask, IDE std_ext, uint32_t filter_mode, uint32_t filter_scale, uint32_t fifo)
{
...
void CANBus::setMBFilter(CAN_BANK bank_num, CAN_FLTEN input)
{
  CAN_FilterTypeDef sFilterConfig;
...
void CANBus::setMBFilter(CAN_FLTEN input)
{
  CAN_FilterTypeDef sFilterConfig;
...
bool CANBus::setMBFilterProcessing(CAN_BANK bank_num, uint32_t filter_id, uint32_t mask, IDE std_ext)
{
  // just convert the MB number enum to bank number.
  return setFilter(uint8_t(bank_num), filter_id, mask, std_ext);
}

bool CANBus::setMBFilter(CAN_BANK bank_num, uint32_t id1, IDE std_ext)
{
...
bool CANBus::setMBFilter(CAN_BANK bank_num, uint32_t id1, uint32_t id2, IDE std_ext)
{
...
void CANBus::initRingBuffer(RingbufferTypeDef &ring, volatile CAN_message_t *buffer, uint32_t size)
{
    ring.buffer = buffer;
    ring.size = size;
    ring.head = 0;
...
bool CANBus::addToRingBuffer(RingbufferTypeDef &ring, const CAN_message_t &msg)
{
    uint16_t nextEntry;
...
bool CANBus::removeFromRingBuffer(RingbufferTypeDef &ring, CAN_message_t &msg)
{
...
bool CANBus::isRingBufferEmpty(RingbufferTypeDef &ring)
{
...
uint32_t CANBus::ringBufferCount(RingbufferTypeDef &ring)
{
    int32_t entries;
...
void CANBus::setBaudRateValues(CAN_HandleTypeDef *CanHandle, uint16_t prescaler, uint8_t timeseg1,
...
bool CANBus::lookupBaudrate(CAN_HandleTypeDef *CanHandle, int baud, const T(&table)[N]) {
...
void CANBus::calculateBaudrate(CAN_HandleTypeDef *CanHandle, int baud)
{
  uint8_t bs1;
  uint8_t bs2;
  uint16_t prescaler;
   ...
   *
   * for more details + justification, see: https://github.com/pazi88/CANBus/pull/41
   */
...
void CANBus::enableMBInterrupts()
{
...
void CANBus::disableMBInterrupts()
{
  if (n_pCanHandle->Instance == CAN1)
  {
